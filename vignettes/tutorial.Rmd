...
devtools::install_github("vanessaaaay/package302", force = TRUE)
```
...
First example -- two.sided t-test:
$$H_a: \mu \neq 60$$
```{r two.sided}
...
second example -- one.sided t-test (greater)
```{r greater}
...
third example -- one.sided t-test (less)
$$H_a: \mu < 60$$
```{r less}
